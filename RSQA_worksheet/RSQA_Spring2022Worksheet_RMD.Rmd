...
fig2_world_map <- ggplot(fig2_world) +
  ...
  theme_bw() +
  labs(title='World Map', x='Longitude', y='Latitude') +
  geom_point(data=fig2_data_joined, aes(DEC_LONG_VA, DEC_LAT_VA, size=RESULT_VA, color=COUNTY_NM.x), alpha=0.45) +
...
fig6_world_map <- ggplot(fig6_world) +
  ...
  labs(title='Map of NESQA Sampling Sites',
       ...
       alpha=NA)+
  coord_sf(xlim=c(min(fig6_data_joined$DEC_LONG_VA), max(fig6_data_joined$DEC_LONG_VA)),
           ylim=c(min(fig6_data_joined$DEC_LAT_VA), max(fig6_data_joined$DEC_LAT_VA)))
fig6_world_map
```
